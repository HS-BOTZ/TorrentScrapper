...
async def link_regex(c,m):
    try:
        ...
        for link in torrent_link:
            ...
            msg += f"**Name : {text}**\n**Link:** {tor}\n\n-\n\n"
        if msg == "":
            await m.send_message(-1001892690797, "No Torrents Found")
            await m.message.delete()
        elif msg != "":
            reply_text = f"{msg}"
            await c.send_photo(-1001892690797, p, caption=heading)
            await c.send_message(-1001892690797, reply_text)
            await txt.delete()

    except Exception as e:
        print(e)
        await c.send_message(-1001892690797, 'Some error occurred')
        await txt.delete()
